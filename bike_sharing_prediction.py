# This project predicts hourly count of bike rentals with respect to environmental and seasonal attributes
...
data = "C:/Users/Gabriel/Desktop/Bike Sharing/hourly_data.csv"
...
# Through inspection, the bike sharing dataset is a multivariate dataset, having categorical and numeric values
# and no missing values. But it requires data scaling. Also, the target attributes contains floating values.

# Feature Engineering:
# The fields dteday, instant and yr will be dropped
# dteday represents date, instant is only an index while yr is not important because years don’t ‘repeat’
# like months or days of the week, so it doesn’t really add a prediction element to future information.
...
# Another problem is that the registered and casual variables. Since the count (cnt) variable is a decomposition
# of these two variables, we could have problems if they remain on the data set. Thus they are dropped too.
...
scale = StandardScaler()# The StandardScaler normalizes the data to a mean of 0 and std of 1.
...
for feature_size in np.arange(2, 18, 2):
    ...
    plt.savefig("bike_without_featuresize_{}".format(feature_size))
    # plt.show()
    print('\n')
