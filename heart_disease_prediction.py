# This project describes the clinical records of heart failure patients to predict if a patient died or not after being released from medical treatment
...
data = "C:/Users/Gabriel/Desktop/heart_failure.csv"
...
# Check target in the data
df_target = df['DEATH_EVENT'].unique()
print(df_target)

# Target distribution among samples
target_dist = df['DEATH_EVENT'].value_counts()
print(target_dist)

# Through inspection, the heart failure dataset is a multivariate dataset, having float and integer feature values
# with no missing values and suffers from outliers. Also, the target attributes include two labels 0 and 1.
# The data can be treated as a classification problem but fed into regression models.
...
print('Death Event-No:',sum(y_train==0))
print('Death Event-Yes:',sum(y_train==1))
...
for feature_size in np.arange(2, 18, 2):
    ...
    plt.savefig("heart_without_featuresize_{}".format(feature_size))
    #plt.show()
    print('\n')
