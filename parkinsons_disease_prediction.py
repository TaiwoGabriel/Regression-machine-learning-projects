# This project predicts the clinical motor and unified Parkinson's disease rating scale (UPDRS) scores of clinical subjects in a parkinsons disease dataset.
...
data = "C:/Users/Gabriel/Desktop/Parkinsons/parkinsons_disease.csv"
df = pd.read_csv(data, delimiter=',')
# Inspect data
print('Inspect Data')
print('Normal Data Shape:',df.shape)
#print(df.head(50).to_string())
df = df.sample(frac=0.14)
df = df.sort_index()
#print(df.head(50).to_string())
...
# Putting the target labels to the last n-1 and nth columns respectively
X = df.loc[:, (df.columns != 'motor_UPDRS') & (df.columns != 'total_UPDRS')]
y = df.loc[:, (df.columns == 'motor_UPDRS') | (df.columns == 'total_UPDRS')]
...
# Put X and y in pandas dataframe to visualize them
X = pd.DataFrame(X)
y = pd.DataFrame(y)
...
# Density plots
X.plot(kind='density', subplots=True, layout=(6,6), sharex=False, legend=True, fontsize=1, figsize=(4,4))
plt.show()
...
for feature_size in np.arange(2, 18, 2):
    ...
    plt.savefig("parkinson_without_featuresize_{}".format(feature_size))
    #plt.show()
    print('\n')
